...
After running `terraform apply`, the deployment creates:
- **27 AWS resources** including VPC, subnets, security groups, EC2 instance, and S3 bucket
- **Public IP address** for EC2 instance to access the web server
- **Private subnets** ready for database deployments
- **S3 bucket** with versioning and encryption enabled
- **Terraform state** stored remotely in S3 with DynamoDB locking

All outputs can be viewed with `terraform output` command after successful deployment.
...
# NOTE: Replace 'terraform-state-<your-bucket-name>' with your actual bucket name
